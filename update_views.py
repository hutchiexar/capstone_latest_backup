...
# Read the file
with open('traffic_violation_system/views.py', 'r') as f:
    content = f.read()

# Find the driver_detail function
driver_detail_pattern = r'@login_required\s*def driver_detail\(request, pk\):(.*?)return redirect\(\'admin_driver_list\'\)'
driver_detail_match = re.search(driver_detail_pattern, content, re.DOTALL)

if driver_detail_match:
    old_function = driver_detail_match.group(0)
    
    # Create the updated function with violations
...
def driver_detail(request, pk):
    ...
    # Update the content
    updated_content = content.replace(old_function, updated_function)
    
    # Write back to the file
    with open('traffic_violation_system/views.py', 'w') as f:
        f.write(updated_content)
    
    print("Successfully updated driver_detail function in views.py")
else:
    print("Could not find driver_detail function in views.py") 